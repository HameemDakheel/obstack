# obstack Jelastic Templates

This directory contains the JPS (Jelastic Packaging Standard) manifests that turn obstack into a one-click install in any Jelastic-based PaaS (mirohost, Hostinger Cloud, Layershift, J-elasticHost, OVH Jelastic Cloud, etc.).

## Files

| File | Type | What it does |
|------|------|--------------|
| `manifest.jps` | Install manifest | Provisions a Docker engine node, clones obstack, generates credentials, runs `make simple` |
| `linker.jps` | Update manifest (add-on) | Injects OTLP env vars into other Jelastic apps so they send telemetry to a co-located obstack environment |
...
3. Pick environment name (defaults to `obstack`), domain, and optional alert webhook.
...
After obstack is running, install `linker.jps` as an **add-on** on each application environment you want to instrument. It injects:
...
- The Jelastic node runs **one** Docker engine; obstack runs as 8 containers inside it.
...
- The manifest pins to a specific obstack git tag (default `v1.0.0-alpha.4`). Bump the `repoRef` setting at install time to deploy a different release.
